...
def _polza_synthetic_models(settings: Any) -> list[ModelCapabilitySnapshot]:
    """Один снимок на модель: если POLZA_VISION_MODEL совпадает с текстовой — помечаем vision на той же записи.

    Иначе `simulate_routing_decision` находит первую строку каталога по имени модели без vision → capability_mismatch.
    """
