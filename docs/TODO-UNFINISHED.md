...
### 23. 🟡 ЧАСТИЧНО 2026-08-04 — Токен вебхука в query string; fail-open при пустом значении

> **Закрыто наполовину заходом 1.** Токен уехал из query string в Basic-auth
> (`http_config.basic_auth` у receiver'а `telegram-admin`) — в логах `admin`
> он больше не появляется. Проверено вживую: верный токен через Basic → 200,
> неверный, отсутствующий и верный-с-чужим-логином → 403.
>
> **Остаётся:** fail-open. Правка в `admin/backend/routers/monitoring.py`
> (пустой токен → 503 вместо молчаливого пропуска) закоммичена, но **не раскатана**:
> код admin запечён в образ и требует пересборки. Сейчас безвредна — токен задан.
