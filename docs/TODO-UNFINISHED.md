...
| | Пункты |
|---|---|
| ✅ **Сделано** — 18 | 1 кластеризация · 2 контур RSI · 4 алертинг · 5 восстановление · **13 счётчики стадий** · **14 метрики хранилищ** · 15 свежесть аналитики · 17 метрики бэкапа · 19 слияние в main · 20 незакоммиченный код · 21 профили compose · 23 токен вебхука · **24 RSI-метрики** · 30 снимок экспозиции · **39 потеря при тримминге** · 44 replay окна · 56 шумящий алерт |
| 🟡 **Частично** — 2 | 18 петля alert-triage (сигнал живости есть, причина смерти нет) · **12 DLQ** (видима, но консьюмера и экрана нет) |
| 🔒 **Отложено решением владельца** — 2 | 3 аутентификация шлюза 8102 · 22 внешняя экспозиция и файрвол |
| ⬜ **Открыто** — 35 | остальные |

**Заходы 7 и 8 маршрута II выполнены.** Следующий — заход 9 (краул и консьюмеры),
и у него уже есть готовая проверка: детектор осиротевшего продюсера из захода 8
прямо сейчас показывает `stream:posts:enriched` с нулём consumer-групп. Заход 9
снимает продюсера, и детектор обязан погаснуть — это проверка не только уборки,
но и самого детектора.
