...
## Что перепроверка ОПРОВЕРГЛА

Две записи, которые не надо чинить, потому что они неверны.

**Ротация docker-логов есть.** Аудит отметил, что `/etc/docker/daemon.json` не содержит
`log-opts` — формально верно, но в `docker-compose.yml` объявлен якорь
`x-docker-json-logging` с `max-size: 10m, max-file: 5`, и он применён ко всем 18 сервисам.
Проверено на живых контейнерах через `docker inspect .HostConfig.LogConfig`.
Не заводить этот долг повторно.

**Знаменатель метрики покрытия кластеризации верен.** Перепроверка утверждала, что
экспортёр считает окно как `GREATEST(semantic, trend)`, а кластеризация работает
в `semantic`. Это не так: `worker/services/semantic_clustering.py:2489` передаёт
в `_fetch_posts` именно `max(semantic_cluster_window_days, trend_cluster_window_days)` —
ровно то, что вычисляет экспортёр. Проверено чтением кода.
