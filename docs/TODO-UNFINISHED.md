...
| | Пунктов | Номера |
|---|---|---|
| ✅ **Сделано** | 34 | 1, 2, 4, 5, 9, 11, 13, 14, 15, 16, 19, 20, 21, 23, 24, 26, 28, 30, 37, 39, 41, 44, 47, **54**, 56, 58, 59, 60, 61, 63, 68, 70, 71, **74** |
...
| ⬜ **Открыто** | 29 | 6, 7, 27, 29, 31, 32, 33, 34, 35, 36, 38, 40, 42, 45, 46, 50, 51, 52, 55, 57, 64, 65, 66, 67, 69, 72, 73, 75, **76** |
...
| Состояние | работает частично 29 · только спроектировано 8 · код есть, не подключён 7 · брошено 3 · подключено, выключено флагом 3 · раскатано, не зафиксировано 1 |
...
### 74. ✅ ЗАКРЫТО 16.08.2026 — слияние сигналов удалено целиком

`enrichment` · закрыто · решение владельца

- [x] **Было.** `_merge_signal_candidates` вызывался на каждом прогоне для stable и emerging, результат писался в `summary.signals_merged`.
- [x] **Не хватало.** Хоть одного слияния: `signals_merged = 0` у всех шести воркспейсов за всю историю наблюдений — ровно как предсказывает арифметика ниже.
- [x] **Что сделано.** Цикл, обе точки вызова, настройки `signal_merge_*` (из `shared/config.py`, `_cluster_settings`, `_thresholds_from_cfg` и переопределения ai_trends в `config/workspaces.yml`) и метрика `signals_merged` удалены. Разбор перенесён в комментарий на месте удаления, в `_signal_results`. Тесты `test_signal_merge_ceiling.py` и `test_merge_signal_candidates_merges_neighboring_duplicates` удалены вместе с функцией, которую стерегли.
- [x] **Почему удалили, а не починили.** Понижать порог до достижимого уровня означало бы слипание сигналов по совпадению слов в заголовке — это ровно то, от чего предупреждал закрытый теперь `test_signal_merge_ceiling.py`. Чинить надо было бы вход, а не порог: пока кандидаты строятся разбиением, любой предикат на пересечении их документов тождественно нулевой.
- [x] **Цена, которую это снимает.** Профиль 16.08.2026: цикл O(k²) при k = 5091 emerging у disruption давал ~13 млн сравнений пар и забирал около 40% времени прогона signal-analysis (сам прогон — 20 мин 17 с при потолке субпроцесса 30 мин).
...
Замер 16.08.2026 (перемер 07.08, база выросла — доли устояли):
...
Всего 1235 из 337 735. По воркспейсам: `auto_hmi` **23.7%** (33 из 139),
design 0.75%, disruption 0.39%, ai_products_media 0.24%, ai_trends и ai_research
по 0.01%. Затронут **21 источник**, из них **18 — на 100%**; тринадцать из этих
восемнадцати живут в `disruption`, где общая доля 0.39% и полностью тонет в
знаменателе. Отсюда и вывод про метрику: она обязана быть per-source.
...
23 поста, все без даты. Свежий случай: `auto_web_ieee_spectrum_autonomous`
(включён 05.08) — 33 поста подряд без даты при 138 прогонах `success`; 16.08
источник переведён на RSS-ленту, и первые же 29 постов после перевода пришли
с датами. То есть **лечение работает, а обнаружение до сих пор было ручным**.

**Уточнение про то, что именно слепо.** `FrontierNoNewPosts` этот отказ не ловит,
но не по той причине, которая напрашивается: `frontier_last_post_age_seconds`
считается по `MAX(COALESCE(published_at, created_at))`
([admin/backend/main.py](../admin/backend/main.py)), поэтому посты без даты для
него существуют и держат возраст свежим — он честно отвечает «данные идут».
Слепа **per-source** метрика `frontier_source_freshness_hours`: она требует
`published_at IS NOT NULL`, и у источника со 100% пропусков **не возникает серии
вообще**. Отсутствие ряда, а не плохое значение — тот же класс, что лечили
у метрик свежести аналитики.

</details>

**Что сделано 16.08.2026 — видимость (сам отказ не лечён).**

- `frontier_source_dateless_ratio{source_id,workspace,source_type,window="7d"|"prior"}`
  и знаменатель `frontier_source_posts_in_window{...}` — из textfile-экспортёра
  `scripts/export-analysis-freshness.sh`, 216 включённых источников.
- Алерт `FrontierSourceDatelessRatioJumped`: доля выше половины **и скачок больше
  0.3 против всей предыдущей истории источника** при знаменателе ≥ 10.
  Перемотка обоих вариантов по реальным данным за 45 суток: порог на **факт** был
  бы firing **46 суток из 46** (116 источнико-суток) — входа, при котором он молчит,
  не существует; со сравнением с историей — **18 суток из 46**, 70 источнико-суток,
  и **ноль на момент внедрения**. Все срабатывания легли в два всплеска (02–12.07
  и 06–12.08), оба совпадают с включением партий новых web-источников.
- `frontier_posts_unanalyzable{workspace,reason}` — осадок по всем трём предикатам
  выборки, без алерта (наблюдаемость).
- Юнит-тесты правила: `prometheus/alerts.test.yml` + `scripts/verify-alert-rules.sh`
  (`promtool test rules`), контракт — `tests/test_dateless_posts_observability.py`.

> **Где искать код: коммит `d685ccd`, и его сообщение врёт.** Всё перечисленное выше
> лежит в коммите `d685ccd db: drop the stub finish times left on 93 retro-closed runs`,
> к содержимому отношения не имеющем. Две сессии работали в одном серверном чекауте:
> первая собрала свои файлы в индекс и не успела закоммитить, вторая сделала
> `git add` своего файла и `git commit` — а `git commit` берёт **весь индекс**, а не
> то, что добавил вызывающий. Коммит уже запушен, переписывать нечего.
> Приём против повторения: `git commit -F - -- <пути>` — с pathspec коммит берёт
> рабочие копии перечисленных файлов и чужой индекс не трогает.
...
<details><summary>Замер варианта (б) на живых данных 16.08.2026 — рекомендация ослабла</summary>

Теневой прогон (только чтение: выборка → векторы из Qdrant → компоненты связности
теми же порогами воркспейса; ни одной записи, LLM не вызывался). Сравнивались
`p.published_at IS NOT NULL` и `COALESCE(p.published_at, p.created_at)`.

**Сколько постов добавится в окно:**

| workspace | окно | потолок | выбрано сейчас | с COALESCE | добавлено без даты | вытеснено потолком |
|---|---|---|---|---|---|---|
| auto_hmi | 30д | 400 | 42 | 61 | **+19 (+45%)** | 0 |
| design | 21д | 650 | 650 | 650 | +9 | 9 |
| ai_trends | 30д | 550 | 550 | 550 | 0 | 0 |
| ai_products_media | 30д | 500 | 500 | 500 | 0 | 0 |
| ai_research | 30д | 500 | 500 | 500 | 0 | 0 |
| disruption | 30д | 2000 | 2000 | 2000 | +18 из 212 | 18 |

У `disruption` в окно попадают 212 постов без даты, но потолок 2000 покрывает
примерно полтора суток притока, поэтому доезжают только 18 — и ровно 18 датированных
из выборки вытесняются. Чистый эффект — обмен 0.9% выборки.

**Как меняется состав кластеров** (auto_hmi, design, ai_trends прогнаны целиком;
disruption не прогонялся — попарный проход по 27 тысячам постов занимает у ночного
джоба порядка десяти минут ядра, а меняется в нём 18 постов из 2000):

| workspace | кластеров | синглтонов | ≥3 док. | ≥2 источников |
|---|---|---|---|---|
| auto_hmi | 38 → **57** | 36 → 55 (94.7% → **96.5%**) | 2 → 2 | 0 → 0 |
| design | 521 → 520 | 475 → 475 (91.2% → 91.4%) | 7 → **8** | 6 → 6 |
| ai_trends | 521 → 521 | 500 → 500 | 4 → 4 | 11 → 11 |

**Главное число: из 19 добавленных постов `auto_hmi` в непустой кластер не попал
ни один — все 19 стали синглтонами.** У `design` из 9 добавленных к кому-то
присоединились 4, и ровно один кластер перевалил за три документа. Порог
`≥2 источника` не сдвинулся нигде.

То есть вариант (б) в его нынешнем виде добавляет корпус, но **почти не добавляет
сигнала**: материал web-источников тематически одинок. Это не аргумент против (б) —
это указание, что от него не надо ждать трендов сам по себе; его настоящая польза
в том, что посты перестают быть невидимыми для поиска по окну и для метрик покрытия.
Если цель — тренды у `auto_hmi`, дешевле сработает (в) плюс расширение набора
источников, а не подстановка даты.

**Побочная находка, к развилке не относящаяся.** У `ai_research` два эквивалентных
запроса (`ORDER BY p.published_at` и `ORDER BY COALESCE(p.published_at, p.created_at)`
при полном отсутствии NULL) дали выборки, различающиеся на **35 постов из 500**:
в ленте много постов с одинаковым timestamp, а `LIMIT` без вторичного ключа
сортировки разрывает ничью произвольно. Значит и два последовательных штатных
прогона кластеризации видят слегка разные выборки. Ущерб не измерялся.
