...
## Состояние на 2026-08-04, вечер

Всего пунктов **55** (52 из аудита + 3 найдено при перепроверке вечером).

| | Пункты |
|---|---|
| ✅ **Сделано** — 9 | 1 кластеризация · 2 контур RSI · 4 алертинг · 5 восстановление · 17 метрики бэкапа · 19 слияние в main · 20 незакоммиченный код · 21 профили compose · 23 токен вебхука |
| 🔒 **Отложено решением владельца** — 1 | 3 аутентификация шлюза 8102 |
| ⬜ **Открыто** — 45 | остальные |
...
⏳ **Две записи опровергнуты ПРЕДВАРИТЕЛЬНО и ждут финальной перепроверки** —
ротация docker-логов и знаменатель метрики покрытия. Оба вердикта держатся на
одном прочтении, а такое прочтение за 04.08 трижды оказывалось неверным.
Процедуры перепроверки — в разделе [«Опровергнуто предварительно»](#-опровергнуто-предварительно--перепроверить-в-конце)
в конце файла. **Реестр нельзя объявлять разобранным, пока эти две не закрыты
измерением.**

Числа внутри пунктов — на момент аудита (утро 04.08). Там, где вечерняя
перепроверка дала другое, стоит пометка «пересняно».
...
### 9. stream:posts:enriched — стрим, в который только пишут: ноль consumer-групп
...
### 17. ✅ СДЕЛАНО 2026-08-04 — ~~У бэкапов и квоты S3 нет ни метрики, ни алерта~~

> **Закрыто заходом 2.** `scripts/export-backup-metrics.sh` в cron пишет свежесть
> бэкапа, размеры артефактов и занятость бакета в textfile-коллектор; добавлена
> группа правил `frontier_backup` (5 штук): протухший бэкап, падающий прогон,
> квота >85%, свободного места меньше одного бэкапа, смерть самого экспортёра.
>
> **Внимание:** алерты по квоте уже `pending` — бакет заполнен на 87.9%, и места
> на следующий ночной прогон не хватает. Это не ложное срабатывание, а ожидающее
> решения [требует решения владельца](./AUDIT-2026-08-04.md#7-маршрут).
...
### 53. crawl4ai — единственный сервис с историей падений, и при этом без healthcheck
...
### 54. Сорок веток в origin, все влиты — мёртвые указатели в листинге
...
## ⏳ Опровергнуто ПРЕДВАРИТЕЛЬНО — перепроверить в конце работ

Две записи, которые по текущей проверке чинить не надо. **Вердикт не окончательный.**

Основание для осторожности прямое: оба опровержения держатся на одном прочтении —
моём, — а за 04.08.2026 такое прочтение трижды оказывалось неверным (правило,
которое не могло сработать; «эталонный» набор профилей; счёт инструментов
регулярками вместо AST). Одиночная проверка одним человеком — ровно тот способ,
которым в этот реестр попадала половина ошибок.

Поэтому: **не чинить сейчас, но и не закрывать.** Перед тем как объявить реестр
разобранным, прогнать процедуры ниже — они механические и занимают минуты.

### R1. Ротация docker-логов

**Предварительный вердикт: есть, долг не подтверждается.** Аудит смотрел
в `/etc/docker/daemon.json`, где `log-opts` действительно нет, — но ротация
объявлена якорем `x-docker-json-logging` в `docker-compose.yml` (`max-size: 10m`,
`max-file: 5`) и применена ко всем 18 сервисам.

Перепроверить так (должно вернуть 18 строк с `max-size:10m`):
...
Если хоть один контейнер отдаст `{}` — ротации у него нет, и пункт живой.
Отдельно проверить размер каталога логов: `du -sh /var/lib/docker/containers`.

### R2. Знаменатель метрики покрытия кластеризации

**Предварительный вердикт: верен.** Вечерняя перепроверка утверждала, что
экспортёр берёт окно как `GREATEST(semantic, trend)`, а кластеризация работает
в `semantic` — то есть знаменатель завышен. По чтению кода это не так:
`worker/services/semantic_clustering.py:2489` передаёт в `_fetch_posts` именно
`max(semantic_cluster_window_days, trend_cluster_window_days)`.

Чтения мало — оно и подвело автора претензии. Перепроверять **измерением**, а не
ещё одним чтением того же файла. Сверить два числа:

1. `post_candidates` из `cluster_runs.summary` последнего успешного прогона —
   сколько постов выборка взяла на самом деле;
2. `frontier_clustering_eligible_posts` и `frontier_clustering_max_posts`
   из textfile-метрик (`prometheus/textfile/frontier_analysis.prom`).

Ожидаемое соотношение при верном знаменателе: `post_candidates` равен
`max_posts`, когда подходящих постов в окне больше потолка (выборка усечена),
и равен числу подходящих, когда их меньше. Если `post_candidates` систематически
**меньше** обоих — значит окно у прогона короче, чем считает экспортёр,
и претензия верна.

SQL для первой цифры (выполнять на сервере в psql, без обёрток):

```sql
SELECT workspace_id, started_at, summary->>'post_candidates' AS post_candidates
FROM cluster_runs
WHERE stage = 'full' AND status = 'success'
