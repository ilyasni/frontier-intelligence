...
> **Приём, купленный ошибкой.** Всё перечисленное выше сначала уехало в ЧУЖОЙ
> коммит: две сессии работали в одном серверном чекауте, первая собрала свои файлы
> в индекс и не успела закоммитить, вторая сделала `git add` своего файла и
> `git commit` — а `git commit` берёт **весь индекс**, а не то, что добавил
> вызывающий. Сообщение коммита потом переписали, но история так и осталась одним
> коммитом на две несвязанные правки — искать код по `git log --stat`, не по теме.
> Против повторения: `git commit -F - -- <пути>` — с pathspec коммит собирается из
> рабочих копий перечисленных файлов и чужой индекс не читает.
