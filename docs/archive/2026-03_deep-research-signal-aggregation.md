...
> **🗄️ ОТЛОЖЕНО В АРХИВ · 2026-08-04.**
> Внешний доимплементационный ресёрч о том, как вообще агрегировать сигналы по визионерству
> и автоиндустрии. Написан **до** Frontier Intelligence, систему не описывает и никогда не описывал —
> ни один файл репозитория на него не ссылался.
>
> Часть рекомендаций была принята и живёт в коде: `feedparser` + `BeautifulSoup` стали основой
> RSS-коннектора, arXiv и Habr вошли в набор источников, а URL `habr.com/ru/rss/flows/featured/`
> зашит в `shared/source_definitions.py` и закреплён юнит-тестом. Остальное — конвейер на кроне
> и половина списка источников — реализовано не было.
>
> Лежал в `docs/` под именем `deep-research-report (6).md` — мусорным именем от скачивания браузером.
> Переименован и отложен, чтобы не читался как проектная документация.
