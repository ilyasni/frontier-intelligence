...
> Конкретных расхождений найдено: **13** — перечислены в разборе.
...
## Что будет при росте до 2000-3000 постов/день

Подробная разбивка — `docs/llm-cost-scenarios.xlsx` лист **Scale_2-3k**. Кратко:

При **2000 постов/день** и сохранении бюджетов токенов:
- Текстовый объём (relevance + valence + mcp + missing): **~234M токенов/мес** (in+out)
- Vision (30% постов, ~18 000/мес): **~54M токенов/мес**
- Embed: **~72M токенов/мес**
- **Итого ~360M токенов/мес**

При **3000 постов/день**: ~540M токенов/мес.

Стоимость в ₽/мес по конфигурациям (только GigaChat + EmbeddingsGigaR, без Wormsoft):

| Конфигурация | 2000/день | 3000/день | Заметка |
|---|---:|---:|---|
| Lite-first (vision вынужденно на Pro) | **77 340** | **116 010** | минимум, но качество concepts/mcp может страдать |
| Recommended mix: Lite + Pro для vision и mcp | 82 290 | 123 435 | +5-7k за качество синтеза в mcp_synthesis |
| Всё на Pro | 147 450 | 221 175 | overpay на относительно простых задачах |
| Всё на Max | **435 150** | **652 725** | абсолютно неоправданно |

Сравнение с гибридом S4 при том же объёме (Wormsoft Simple для текста + Pro для vision premium 30%) — Wormsoft Simple за 1500₽/мес держит **~72M кредитов**, чего хватает на текстовую обработку 2000-3000 постов/день с большим запасом. Гибрид S4 при 3000/день обходится в районе **9-12k ₽/мес** против **116k+ ₽/мес** на чистом GigaChat.

**Вывод:** при росте объёма экономика гибрида растёт нелинейно — Wormsoft Simple остаётся флагман по «стоимость за токен», а GigaChat Pro нужен только там, где Lite физически не справляется (vision) или нужно качество (mcp_synthesis).
