...
# Cron принадлежит пользователю ilyasni, поэтому ему должен быть доступен только
# watchdog credential. Каноническая матрица permissions — в
# docs/runbooks/alert-triage-daily.md. Не переносить этот пользовательский скрипт
# в root-cron ради обхода permissions.
#
