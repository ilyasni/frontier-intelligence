...
#       Скачать бэкап из S3, если локальной копии уже нет.
#
#       ВНИМАНИЕ ПРО ГЛУБИНУ. В S3 срок хранения backups/ — ОДНИ СУТКИ
#       (снижен с двух 04.08.2026: два набора не помещались в квоту 15 GiB,
#       см. scripts/s3_lifecycle_apply.py). Expiration отрабатывает асинхронно,
#       поэтому фактически лежит два набора: вчерашний и сегодняшний. Достать
#       что-либо старше вчера этим режимом НЕЛЬЗЯ.
#       Локальная копия в backups/ живёт дольше, но лежит на том же хосте,
#       что и данные, то есть офсайтом не является.
#
#       ВНИМАНИЕ ПРО СОСТАВ (21.08.2026). В S3 уезжает НЕ ВЕСЬ комплект: маски
#       из S3_UPLOAD_EXCLUDE (.env) оставляют тяжёлый снапшот dense-корпуса
#       Qdrant только локально — он производный и переиндексируется из
#       postgres.dump, а место в квоте нужнее единственному незаменимому файлу.
#       Что именно не уехало, написано в MANIFEST.txt внутри самого набора,
#       и режим verify об этом говорит отдельной строкой.
#
...
do_verify() {
  ...
  # Отсутствие ПО ЗАМЫСЛУ — не порча, и путать их нельзя в обе стороны. Набор,
  # скачанный из S3, законно не содержит исключённого из выгрузки; при этом
  # проверка выше сказала бы "снапшоты на месте", увидев один лишь мелкий
  # trend_clusters, и умолчала бы о том, что корпуса нет. Манифест едет вместе
  # с набором именно для этого ответа.
  if [ -f "$DIR/MANIFEST.txt" ] && grep -q "^S3_UPLOAD_EXCLUDE=" "$DIR/MANIFEST.txt"; then
    log "  вне S3 по замыслу: $(grep "^S3_UPLOAD_EXCLUDE=" "$DIR/MANIFEST.txt" | cut -d= -f2-)"
    log "  -> эти файлы ищи в локальном каталоге бэкапа; векторы иначе восстанавливаются переиндексацией из postgres"
  fi
