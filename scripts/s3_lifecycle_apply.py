...
#   backups/   1д  — ночные дампы стека (~3.56 GiB/день); интерим под квоту бакета ~15 GiB.
#                    Было 2д и не сходилось по арифметике. Expiration отрабатывает
#                    асинхронно, раз в сутки, поэтому при сроке N суток в бакете
#                    в пике лежит N+1 набор: при 2д — три по 3.5 GiB = 10.6 GiB,
#                    и вместе с media/ и crawl/ это 13.19 из 15.00 GiB. Свободного
#                    места оставалось 1.81 GiB при потребности 3.56 — то есть
#                    следующая ночная выгрузка гарантированно падала. Замерено
#                    04.08.2026, когда у бэкапов впервые появились метрики.
#                    При 1д в пике два набора (~7.1 GiB), заполнение ~64%.
#                    Глубина хранения от этого почти не страдает: локально
#                    backup-stack.sh держит 7 суток, сокращается только offsite.
#                    Держать дольше — поднять квоту бакета ИЛИ исключить
#                    dense_2560-снапшоты (2.4 GiB, 68% веса бэкапа) из S3-заливки.
